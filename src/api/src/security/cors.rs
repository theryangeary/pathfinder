...
fn handle_preflight(config: &SecurityConfig, headers: &HeaderMap) -> Response {
    ...
    if let Some(origin_value) = origin {
        if !is_origin_allowed(origin_value, &config.allowed_origins) {
            warn!(
                "CORS Preflight: Blocked request from unauthorized origin: {}",
                origin_value
            );
    ...
    if let Some(method) = requested_method {
        if !allowed_methods.contains(&method) {
            warn!(
                "CORS Preflight: Blocked request with unauthorized method: {}",
                method
            );
            return Response::builder()
                .status(StatusCode::FORBIDDEN)
    ...
    if let Some(headers_str) = requested_headers {
        for header in headers_str.split(',') {
            ...
            if !allowed_headers.contains(&header.as_str()) {
                warn!(
                    "CORS Preflight: Blocked request with unauthorized header: {}",
                    header
                );
                return Response::builder()
                    .status(StatusCode::FORBIDDEN)
...
fn add_preflight_headers(config: &SecurityConfig, headers: &mut HeaderMap, origin: Option<&str>) {
    ...
    headers.insert(
        ...
        HeaderValue::from_static("Content-Type, X-Requested-With, X-CSRF-Token"),
    );
    headers.insert(
        "access-control-allow-credentials",
        HeaderValue::from_static("true"),
    ...
    headers.insert(
        ...
        HeaderValue::from_str(&config.cors_max_age.as_secs().to_string()).unwrap(),
    );
    headers.insert("vary", HeaderValue::from_static("Origin"));
}
