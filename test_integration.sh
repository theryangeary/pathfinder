...
today=$(date +%Y-%m-%d)
game_response=$(curl -s http://localhost:3001/api/game/date/$today)
...
if [ -n "$game_id" ]; then
    ...
    words_response=$(curl -s http://localhost:3001/api/game/$game_id/words)
    ...
    paths_response=$(curl -s http://localhost:3001/api/game/$game_id/paths)
    ...
    if [ -n "$user_id" ] && [ -n "$cookie_token" ]; then
        entry_response=$(curl -s "http://localhost:3001/api/game-entry/$game_id?user_id=$user_id&cookie_token=$cookie_token")
